...
export class Planner {
  ...
  private async llmPlan(input: PlannerInput): Promise<PlannerOutput> {
    try {
...
`);
  ...
  private async ruleBasedPlan(input: PlannerInput): Promise<PlannerOutput> {
    ...
    const oldestGoal = incompleteGoals.reduce((oldest, current) => 
  ...
  async evaluateGoalCompletion(goal: AgentGoal, taskHistory: AgentTask[]): Promise<boolean> {
    if (this.llm) {
  ...
  private async llmEvaluateGoal(goal: AgentGoal, taskHistory: AgentTask[]): Promise<boolean> {
    try {
      const prompt = PromptTemplate.fromTemplate(`
...
`);
  ...
  private async ruleBasedEvaluateGoal(goal: AgentGoal, taskHistory: AgentTask[]): Promise<boolean> {
    ...
    return recentSuccessfulTasks.some(task => 
      task.description.toLowerCase().includes(goal.description.toLowerCase()) ||
